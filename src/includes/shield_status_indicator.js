...
class ShieldStatus extends React.Component {
    ...
    componentDidMount() {
        ...
        //but keep it refreeshing every 30 minutes in case they leave their computer
        this.interval = setInterval(() => this.getShieldStatus(), 1800000);
    }
    ...
    enableShield = async () => {
        ...
        try {
            await fetch('activate/', {method: 'POST'}).then(() => {
                this.getShieldStatus();
            });
    ...
    disableShield = async () => {
        ...
        try {
            fetch('deactivate/', {method: 'POST'}).then(() => {
                this.getShieldStatus();
            });
        } catch (err) {
